...
- [ ] **STRM-07**: HLS files are written to tmpfs (`RuntimeDirectory=stream`) and served by MediaMTX on its HTTP origin port with correct cache headers (`.m3u8` 1s, `.ts` 1 day immutable).
...
- [ ] **INFRA-03**: TLS termination on the droplet via Let's Encrypt with auto-renewal.
