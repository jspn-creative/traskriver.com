...
- `stripe` `^20.4.1` — payments and webhooks (`packages/web/src/lib/server/stripe.ts`, `packages/web/src/routes/api/stripe/*`).
...
- SvelteKit: private env via `$env/dynamic/private` (`packages/web/src/lib/server/stripe.ts`, `packages/web/src/routes/stream.remote.ts`).
