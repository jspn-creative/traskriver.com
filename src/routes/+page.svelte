	...
	<div class="mx-auto flex min-h-screen w-full max-w-6xl flex-col justify-center gap-10 px-6 py-10">
		<div class="space-y-4">
			...
			<p class="max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
				Live stream via Cloudflare Stream. Access is automatically granted.
			</p>
		</div>
		...
		<div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
			...
			<div class="rounded-3xl border border-white/10 bg-white/5 p-6">
				...
				<p class="mt-3 text-sm leading-6 text-slate-300">
					The player is using Cloudflare Stream. Run `bun run push-stream` from the camera location
					to start ingesting.
				</p>
			</div>
		</div>
