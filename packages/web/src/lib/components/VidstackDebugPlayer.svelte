<script lang="ts">
	...
	$effect(() => {
		...
		const emitDiagnostic = (type: string, detail?: Record<string, unknown>) => {
			const diagnostic = untrack(() => ({ ...snapshot(el, nativeVideo, startedAt, detail), type }));
		...
		const emitStartupCheck = (type: string) => {
			...
			emitDiagnostic(type, {
				src: liveSrc,
				capabilities: collectStreamCapabilities(nativeVideo),
		...
		const attachNativeVideo = (video: NativeVideoElement | null) => {
			...
			emitDiagnostic('native_video_attached', {
				capabilities: collectStreamCapabilities(video)
			});
