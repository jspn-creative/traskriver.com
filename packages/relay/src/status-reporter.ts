...
export class StatusReporter {
  constructor(private config: { statusApiUrl: string; bearerToken: string; requestTimeoutMs: number }) {}

  async report(state: string) {
    try {
      const payload = { state, timestamp: Date.now() } satisfies RelayStatusPayload;
